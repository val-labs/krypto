...
int main(int argc, char*argv[]) {
  ...
  int size = 1024*1024; /* make a nice big buffer, 1M */
  unsigned char*buf=malloc(size);
  int bytes = fread(buf, 1, size, stdin);
  if(bytes < 0) PERR("<stdin>: ");
  if(bytes < 1) DIE("file reading error.  (is it blank?)");
  switch(action){
  case 'V':   
  ...
  default: DIE("no action: try -s or -v");}}
